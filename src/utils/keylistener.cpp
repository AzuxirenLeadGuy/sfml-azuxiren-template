...
utils::KeyListener::KeyListener(std::initializer_list<sf::Keyboard::Key> arg_keys)
    : keys(arg_keys.size()), values(arg_keys.size())
{
...
auto utils::KeyListener::Find(int code) -> int
{
	...
	while (keys[idx] != code && idx < limit) { idx++; }
...
auto utils::KeyListener::FrameEnd() -> int
{
	...
	{
		auto state = values[i];
		if (state == KeyState::Just_Pressed) { state = KeyState::Long_Pressed; }
		if (state == KeyState::Just_Release) { state = KeyState::Long_Release; }
		values[i] = state;
	}
	return 0;
}
